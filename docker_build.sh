...
usage() {
    ...
    echo "     -n network profile, options: devnet, testnet2, mainnet, testnet"
...
if [ x"$NETWORK" == x"devnet" ] || [ x"$NETWORK" == x"testnet" ] || [ x"$NETWORK" == x"mainnet" ] || [ x"$NETWORK" == x"testnet2" ]; then
    IMAGEID="$IMAGEID:$NETWORK"
else
    echo "invalid network option, use 'testnet' instead"
    IMAGEID="$IMAGEID:testnet"
fi
if [ $PROXY -eq 1 ]; then
